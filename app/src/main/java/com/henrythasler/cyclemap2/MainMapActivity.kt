        ...
        override fun run() {
            if (trackRecording && locationServiceBound) {
                ...
                timerHandler.postDelayed(
                    this,
                    min(TRACK_RECORDING_INTERVAL, ODOMETER_UPDATE_INTERVAL)
                )
...
//                puckBearingSource = PuckBearingSource.HEADING
    ...
    private fun onSearchButton() {
        ...
        if (sheet.state == BottomSheetBehavior.STATE_EXPANDED) {
            val imm =
                mapView.context.getSystemService(Context.INPUT_METHOD_SERVICE) as InputMethodManager
    ...
    private fun closeSearchView() {
        BottomSheetBehavior.from(findViewById(R.id.geosearch)).state =
